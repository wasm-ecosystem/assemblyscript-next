...
export class Flow {
    ...
    if (!scopedLocals) {
      ...
      if (reportNode) {
        if (!existingLocal.declaration.range.source.isNative) {
          this.program.errorRelated(DiagnosticCode.Duplicate_identifier_0, reportNode.range, existingLocal.declaration.name.range, name);
        } else {
  ...
  mergeBranch(other: Flow): void {
    ...
    for (let i = 0; i < maxLocalFlags; ++i) {
      ...
      thisLocalFlags[i] = thisFlags & otherFlags & (LocalFlags.Constant | LocalFlags.Wrapped | LocalFlags.NonNull | LocalFlags.Initialized);
    }
  ...
  inheritAlternatives(left: Flow, right: Flow): void {
    ...
    if (leftFlags & FlowFlags.Terminates) {
      ...
      for (let i = 0; i < maxLocalFlags; ++i) {
        ...
        thisLocalFlags[i] = leftFlags & rightFlags & (LocalFlags.Constant | LocalFlags.Wrapped | LocalFlags.NonNull | LocalFlags.Initialized);
      }
    }
      ...
      case ExpressionId.Binary: {
        switch (getBinaryOp(expr)) {
          ...
          case BinaryOp.MulI32: {
            return !(
              (getExpressionId((operand = getBinaryLeft(expr))) == ExpressionId.Const &&
                (getConstValueI32(operand) == 0 || (getConstValueI32(operand) == 1 && !this.canOverflow(getBinaryRight(expr), type)))) ||
              (getExpressionId((operand = getBinaryRight(expr))) == ExpressionId.Const &&
                (getConstValueI32(operand) == 0 || (getConstValueI32(operand) == 1 && !this.canOverflow(getBinaryLeft(expr), type))))
            );
          }
          ...
          case BinaryOp.AndI32: {
            ...
            return !(
              (getExpressionId((operand = getBinaryLeft(expr))) == ExpressionId.Const && getConstValueI32(operand) <= type.computeSmallIntegerMask(Type.i32)) ||
          ...
          case BinaryOp.ShlI32: {
            ...
            return getExpressionId((operand = getBinaryRight(expr))) != ExpressionId.Const || getConstValueI32(operand) < shift;
          }
          ...
          case BinaryOp.ShrI32: {
            ...
            return (
              ...
              (getExpressionId((operand = getBinaryRight(expr))) != ExpressionId.Const || getConstValueI32(operand) < shift)
            );
          }
          ...
          case BinaryOp.ShrU32: {
            ...
            return type.isSignedIntegerValue
              ? !(
                  (getExpressionId((operand = getBinaryRight(expr))) == ExpressionId.Const && getConstValueI32(operand) > shift) // must clear MSB
                )
              : this.canOverflow(getBinaryLeft(expr), type) &&
                  !(
                    (getExpressionId((operand = getBinaryRight(expr))) == ExpressionId.Const && getConstValueI32(operand) >= shift) // can leave MSB
                  );
          }
