...
export class Type {
  ...
  isStrictlyAssignableTo(target: Type, signednessIsRelevant: bool = false): bool {
    ...
    if (this.isIntegerValue) {
      return target.isIntegerValue && target.size == this.size && (!signednessIsRelevant || this.isSignedIntegerValue == target.isSignedIntegerValue);
    }
  ...
  toString(validWat: bool = false): string {
    ...
    if (this.isReference) {
      ...
      if (classReference) {
        ...
        let signatureReference = this.getSignature();
        if (signatureReference) {
          return this.isNullableReference ? `(${signatureReference.toString(validWat)})${nullablePostfix}` : signatureReference.toString(validWat);
        } else {
  ...
  /** An 8-bit signed integer. */
  static readonly i8: Type = new Type(TypeKind.I8, TypeFlags.Signed | TypeFlags.Short | TypeFlags.Integer | TypeFlags.Value, 8);

  /** A 16-bit signed integer. */
  static readonly i16: Type = new Type(TypeKind.I16, TypeFlags.Signed | TypeFlags.Short | TypeFlags.Integer | TypeFlags.Value, 16);
  ...
  static readonly isize32: Type = new Type(TypeKind.Isize, TypeFlags.Signed | TypeFlags.Integer | TypeFlags.Varying | TypeFlags.Value, 32);

  /** A 64-bit signed size. WASM64 only. */
  static readonly isize64: Type = new Type(TypeKind.Isize, TypeFlags.Signed | TypeFlags.Long | TypeFlags.Integer | TypeFlags.Varying | TypeFlags.Value, 64);

  /** An 8-bit unsigned integer. */
  static readonly u8: Type = new Type(TypeKind.U8, TypeFlags.Unsigned | TypeFlags.Short | TypeFlags.Integer | TypeFlags.Value, 8);

  /** A 16-bit unsigned integer. */
  static readonly u16: Type = new Type(TypeKind.U16, TypeFlags.Unsigned | TypeFlags.Short | TypeFlags.Integer | TypeFlags.Value, 16);
  ...
  static readonly usize32: Type = new Type(TypeKind.Usize, TypeFlags.Unsigned | TypeFlags.Integer | TypeFlags.Varying | TypeFlags.Value, 32);

  /** A 64-bit unsigned size. WASM64 only. */
  static readonly usize64: Type = new Type(TypeKind.Usize, TypeFlags.Unsigned | TypeFlags.Long | TypeFlags.Integer | TypeFlags.Varying | TypeFlags.Value, 64);

  /** A 1-bit unsigned integer. */
  static readonly bool: Type = new Type(TypeKind.Bool, TypeFlags.Unsigned | TypeFlags.Short | TypeFlags.Integer | TypeFlags.Value, 1);
  ...
  static readonly stringview_wtf8: Type = new Type(TypeKind.StringviewWTF8, TypeFlags.External | TypeFlags.Reference, 0);

  /** Non-nullable WTF16 string view reference (`ref stringview_wtf16`). */
  static readonly stringview_wtf16: Type = new Type(TypeKind.StringviewWTF16, TypeFlags.External | TypeFlags.Reference, 0);

  /** Non-nullable string iterator reference (`ref stringview_iter`). */
  static readonly stringview_iter: Type = new Type(TypeKind.StringviewIter, TypeFlags.External | TypeFlags.Reference, 0);
...
export class Signature {
  ...
  isAssignableTo(target: Signature, checkCompatibleOverride: bool = false): bool {
    ...
    if (thisThisType && targetThisType) {
      const compatibleThisType = checkCompatibleOverride ? thisThisType.canExtendOrImplement(targetThisType) : targetThisType.isAssignableTo(thisThisType);
  ...
  clone(requiredParameters: i32 = this.requiredParameters, hasRest: bool = this.hasRest): Signature {
    ...
    return Signature.create(this.program, cloneParameterTypes, this.returnType, this.thisType, requiredParameters, hasRest);
  }
}
