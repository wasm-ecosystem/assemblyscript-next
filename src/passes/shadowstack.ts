...
function matchPattern(module: Module, expr: ExpressionRef): ExpressionRef {
  let isFound = false;
  while (_BinaryenExpressionGetId(expr) == ExpressionId.Call && module.readStringCached(_BinaryenCallGetTarget(expr)) == BuiltinNames.tostack) {
...
export class ShadowStackPass extends Pass {
  ...
  ptrConst(value: i32): ExpressionRef {
    return this.ptrType == TypeRef.I64 ? this.module.i64(value) : this.module.i32(value);
  }
  ...
  makeStackFill(frameSize: i32, stmts: ExpressionRef[]): void {
    ...
    if (this.options.hasFeature(Feature.BulkMemory) && frameSize > 16) {
      ...
      while (remain >= 8) {
        ...
        stmts.push(module.store(8, module.global_get(BuiltinNames.stack_pointer, this.ptrType), module.i64(0), TypeRef.I64, frameSize - remain));
        remain -= 8;
      }
      if (remain) {
        assert(remain == 4);
        // store<i32>(__stack_pointer, 0, frameSize - remain)
        stmts.push(module.store(4, module.global_get(BuiltinNames.stack_pointer, this.ptrType), module.i32(0), TypeRef.I32, frameSize - remain));
      }
    }
  }
  ...
  makeStackCheck(): ExpressionRef {
    ...
    if (!this.hasStackCheckFunction) {
      ...
      module.addFunction(
        ...
        null,
        module.if(
          module.binary(BinaryOp.LtI32, module.global_get(BuiltinNames.stack_pointer, this.ptrType), module.global_get(BuiltinNames.data_end, this.ptrType)),
          this.compiler.makeStaticAbort(this.compiler.ensureStaticString("stack overflow"), Source.native),
        ),
      );
    }
  ...
  updateExport(exportRef: ExportRef, managedOperandIndices: i32[]): void {
    ...
    if (_BinaryenGetFunction(moduleRef, wrapperNameRef) == 0) {
      ...
      let cArr = allocPtrArray(vars);
      _BinaryenAddFunction(moduleRef, wrapperNameRef, params, results, cArr, vars.length, module.block(null, stmts, results));
      _free(cArr);
    }
