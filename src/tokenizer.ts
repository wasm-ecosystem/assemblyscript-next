...
export class Tokenizer extends DiagnosticEmitter {
  ...
  constructor(source: Source, diagnostics: DiagnosticMessage[] | null = null) {
    ...
    // skip shebang
    if (pos + 1 < end && text.charCodeAt(pos) == CharCode.Hash && text.charCodeAt(pos + 1) == CharCode.Exclamation) {
      pos += 2;
  ...
  private unsafeNext(identifierHandling: IdentifierHandling = IdentifierHandling.Default, maxTokenLength: i32 = i32.MAX_VALUE): Token {
    ...
    while (pos < end) {
      ...
      switch (c) {
        case CharCode.CarriageReturn: {
          if (!(++pos < end && text.charCodeAt(pos) == CharCode.LineFeed)) break;
        ...
        case CharCode.Exclamation: {
          ++pos;
          if (maxTokenLength > 1 && pos < end && text.charCodeAt(pos) == CharCode.Equals) {
            ++pos;
            if (maxTokenLength > 2 && pos < end && text.charCodeAt(pos) == CharCode.Equals) {
              this.pos = pos + 1;
        ...
        case CharCode.Dot: {
          ...
          if (maxTokenLength > 1 && pos < end) {
            ...
            if (maxTokenLength > 2 && pos + 1 < end && chr == CharCode.Dot && text.charCodeAt(pos + 1) == CharCode.Dot) {
              this.pos = pos + 2;
          ...
          if (maxTokenLength > 1 && pos < end) {
            ...
            if (chr == CharCode.Asterisk) {
              ...
              while (++pos < end) {
                ...
                if (c == CharCode.Asterisk && pos + 1 < end && text.charCodeAt(pos + 1) == CharCode.Slash) {
                  pos += 2;
                  closed = true;
                  break;
                }
          ...
          if (maxTokenLength > 1 && pos < end) {
            ...
            if (chr == CharCode.GreaterThan) {
              ...
              if (maxTokenLength > 2 && pos < end) {
                ...
                if (chr == CharCode.GreaterThan) {
                  ++pos;
                  if (maxTokenLength > 3 && pos < end && text.charCodeAt(pos) == CharCode.Equals) {
                    this.pos = pos + 1;
        ...
        default: {
          ...
          if (isIdentifierStart(c)) {
            let posBefore = pos;
            while ((pos += numCodeUnits(c)) < end && isIdentifierPart((c = <i32>text.codePointAt(pos)))) {
              /* nop */
            }
            if (identifierHandling != IdentifierHandling.Always) {
              let maybeKeywordToken = tokenFromKeyword(text.substring(posBefore, pos));
              if (maybeKeywordToken != Token.Invalid && !(identifierHandling == IdentifierHandling.Prefer && tokenIsAlsoIdentifier(maybeKeywordToken))) {
                this.pos = pos;
  ...
  peek(identifierHandling: IdentifierHandling = IdentifierHandling.Default, maxCompoundLength: i32 = i32.MAX_VALUE): Token {
  ...
  readHexInteger(): i64 {
    ...
    while (pos < end) {
      ...
      if (isDecimal(c)) {
        ...
        nextValue = i64_add(i64_shl(value, i64_4), i64_new((c | 32) + (10 - CharCode.a)));
      } else if (c == CharCode._) {
        if (sepEnd == pos) {
          this.error(
            sepEnd == start ? DiagnosticCode.Numeric_separators_are_not_allowed_here : DiagnosticCode.Multiple_consecutive_numeric_separators_are_not_permitted,
            this.range(pos),
          );
        }
        sepEnd = pos + 1;
  ...
  readDecimalFloat(): f64 {
    ...
    if (this.pos < end) {
      let c = text.charCodeAt(this.pos);
      if ((c | 32) == CharCode.e) {
        if ((++this.pos < end && (c = text.charCodeAt(this.pos)) == CharCode.Minus) || (c == CharCode.Plus && isDecimal(text.charCodeAt(this.pos + 1)))) {
          ++this.pos;
        }
        sepCount += this.readDecimalFloatPartial();
      }
  ...
  private readExtendedUnicodeEscape(startIfTaggedTemplate: i32 = -1): string {
    ...
    if (invalid) {
      return ~startIfTaggedTemplate ? text.substring(startIfTaggedTemplate, this.pos) : "";
    }
