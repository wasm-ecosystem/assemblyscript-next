  ...
  while (pos + 1 < len) {
    ...
    if (path.charCodeAt(pos) == separator && path.charCodeAt(pos + 1) == CharCode.Dot) {
      ...
      if (atEnd || (pos + 2 < len && path.charCodeAt(pos + 2) == separator)) {
        path = atEnd ? path.substring(0, pos) : path.substring(0, pos) + path.substring(pos + 2);
        len -= 2;
        continue;
      }
      ...
      if (
        ...
        while (--ipos >= 0) {
          if (path.charCodeAt(ipos) == separator) {
            if (pos - ipos != 3 || path.charCodeAt(ipos + 1) != CharCode.Dot || path.charCodeAt(ipos + 2) != CharCode.Dot) {
              // exclude '..' itself
              path = atEnd ? path.substring(0, ipos) : path.substring(0, ipos) + path.substring(pos + 3);
              len -= pos + 3 - ipos;
