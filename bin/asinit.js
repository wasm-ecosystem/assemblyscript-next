...
console.log(
  [
    "Version: " + version,
    "",
    stdoutColors.white(["This command will make sure that the following files exist in the project", "directory '" + projectDir + "':"].join("\n")),
...
function ensureEntryFile() {
  ...
  if (!fs.existsSync(entryFile)) {
    fs.writeFileSync(
      entryFile,
      ["// The entry file of your WebAssembly module.", "", "export function add(a: i32, b: i32): i32 {", "  return a + b;", "}"].join("\n") + "\n",
    );
...
function ensureGitignore() {
  ...
  if (!fs.existsSync(gitignoreFile)) {
    fs.writeFileSync(gitignoreFile, ["*", "!.gitignore"].join("\n") + "\n");
...
function ensurePackageJson() {
  ...
  if (!fs.existsSync(packageFile)) {
    ...
    if (!scripts["start"]) {
      (scripts["start"] = "npx serve ."), (pkg["scripts"] = scripts);
      updated = true;
    }
...
function ensureTestsIndexJs() {
  ...
  if (!fs.existsSync(testsIndexFile)) {
    fs.writeFileSync(
      testsIndexFile,
      ['import assert from "assert";', 'import { add } from "../build/debug.js";', "assert.strictEqual(add(1, 2), 3);", 'console.log("ok");'].join("\n") + "\n",
    );
