...
export function SORT<T>(ptr: usize, len: i32, comparator: Comparator<T>): void {
  ...
  while (endA < hi) {
    ...
    for (let i = top; i > k; --i) {
      ...
      if (start != EMPTY) {
        mergeRuns<T>(ptr, start, load<u32>(leftRunEndBuf + ((<usize>i) << alignof<u32>())) + 1, endA, buffer, comparator);
        startA = start;
        store<u32>(leftRunStartBuf + ((<usize>i) << alignof<u32>()), EMPTY);
      }
    }
  ...
  for (let i = top; i != 0; --i) {
    let start = load<u32>(leftRunStartBuf + ((<usize>i) << alignof<u32>()));
    if (start != EMPTY) {
      mergeRuns<T>(ptr, start, load<u32>(leftRunEndBuf + ((<usize>i) << alignof<u32>())) + 1, hi, buffer, comparator);
    }
  }
  // dealloc aux buffers
  __free(buffer);
...
function insertionSort<T>(ptr: usize, left: i32, right: i32, presorted: i32, comparator: Comparator<T>): void {
  if (ASC_SHRINK_LEVEL >= 1) {
...
function extendRunRight<T>(ptr: usize, i: i32, right: i32, comparator: Comparator<T>): i32 {
  if (i == right) return i;
  let j = i;
  if (comparator(load<T>(ptr + ((<usize>j) << alignof<T>())), load<T>(ptr + ((<usize>++j) << alignof<T>()))) > 0) {
    while (
      j < right &&
      comparator(load<T>(ptr + ((<usize>j) << alignof<T>()), 1 << alignof<T>()), load<T>(ptr + ((<usize>j) << alignof<T>()))) >>> 31 // < 0
    )
      ++j;
    // reverse
    let k = j;
    while (i < k) {
      ...
      store<T>(ptr + ((<usize>i) << alignof<T>()), load<T>(ptr + ((<usize>k) << alignof<T>())));
      ++i;
      store<T>(ptr + ((<usize>k) << alignof<T>()), tmp);
      --k;
    }
  } else {
    while (j < right && comparator(load<T>(ptr + ((<usize>j) << alignof<T>()), 1 << alignof<T>()), load<T>(ptr + ((<usize>j) << alignof<T>()))) >= 0) ++j;
  }
  return j;
}

// Merges arr[l..m - 1] and arr[m..r]
function mergeRuns<T>(ptr: usize, l: i32, m: i32, r: i32, buffer: usize, comparator: Comparator<T>): void {
  --m;
  let i: i32,
    j: i32,
    t = r + m;
