...
// @ts-ignore: decorator
@lazy @inline const POWERS10 = memory.data<u32>([1, 10, 100, 1000, 10000, 100000, 1000000, 10000000, 100000000, 1000000000]);
...
@lazy @inline const DIGITS = memory.data<u32>([
  0x00300030, 0x00310030, 0x00320030, 0x00330030, 0x00340030, 0x00350030, 0x00360030, 0x00370030, 0x00380030, 0x00390030, 0x00300031, 0x00310031, 0x00320031,
  0x00330031, 0x00340031, 0x00350031, 0x00360031, 0x00370031, 0x00380031, 0x00390031, 0x00300032, 0x00310032, 0x00320032, 0x00330032, 0x00340032, 0x00350032,
  0x00360032, 0x00370032, 0x00380032, 0x00390032, 0x00300033, 0x00310033, 0x00320033, 0x00330033, 0x00340033, 0x00350033, 0x00360033, 0x00370033, 0x00380033,
  0x00390033, 0x00300034, 0x00310034, 0x00320034, 0x00330034, 0x00340034, 0x00350034, 0x00360034, 0x00370034, 0x00380034, 0x00390034, 0x00300035, 0x00310035,
  0x00320035, 0x00330035, 0x00340035, 0x00350035, 0x00360035, 0x00370035, 0x00380035, 0x00390035, 0x00300036, 0x00310036, 0x00320036, 0x00330036, 0x00340036,
  0x00350036, 0x00360036, 0x00370036, 0x00380036, 0x00390036, 0x00300037, 0x00310037, 0x00320037, 0x00330037, 0x00340037, 0x00350037, 0x00360037, 0x00370037,
  0x00380037, 0x00390037, 0x00300038, 0x00310038, 0x00320038, 0x00330038, 0x00340038, 0x00350038, 0x00360038, 0x00370038, 0x00380038, 0x00390038, 0x00300039,
  0x00310039, 0x00320039, 0x00330039, 0x00340039, 0x00350039, 0x00360039, 0x00370039, 0x00380039, 0x00390039,
]);
...
@lazy @inline const EXP_POWERS = memory.data<i16>([
  /* eslint-disable indent */ -1220, -1193, -1166, -1140, -1113, -1087, -1060, -1034, -1007, -980, -954, -927, -901, -874, -847, -821, -794, -768, -741, -715,
  -688, -661, -635, -608, -582, -555, -529, -502, -475, -449, -422, -396, -369, -343, -316, -289, -263, -236, -210, -183, -157, -130, -103, -77, -50, -24, 3,
  30, 56, 83, 109, 136, 162, 189, 216, 242, 269, 295, 322, 348, 375, 402, 428, 455, 481, 508, 534, 561, 588, 614, 641, 667, 694, 720, 747, 774, 800, 827, 853,
  880, 907, 933, 960, 986, 1013, 1039, 1066,
...
function utoa_hex_lut(buffer: usize, num: u64, offset: usize): void {
  ...
  while (offset >= 2) {
    offset -= 2;
    store<u32>(buffer + (offset << 1), load<u32>(lut + (((<usize>num) & 0xff) << alignof<u32>())));
    num >>= 8;
  }
  if (offset & 1) {
  ...
  if (isSigned<T>()) {
    ...
    if (sign) {
      if (sizeof<T>() == 1) {
        if (value == -0x80) {
          // -0x80  ->  -128
          store<u64>(buffer, (<u64>CharCode.MINUS) | ((<u64>(CharCode._0 + 1)) << 16) | ((<u64>(CharCode._0 + 2)) << 32) | ((<u64>(CharCode._0 + 8)) << 48));
          return 4;
        }
      }
      if (sizeof<T>() == 2) {
        if (value == -0x8000) {
          // -0x8000  ->  -32768
          store<u64>(buffer, (<u64>CharCode.MINUS) | ((<u64>(CharCode._0 + 3)) << 16) | ((<u64>(CharCode._0 + 2)) << 32) | ((<u64>(CharCode._0 + 7)) << 48)); // -327
          store<u32>(buffer + 8, ((CharCode._0 + 6) << 0) | ((CharCode._0 + 8) << 16)); // 68
          return 6;
        }
      }
      store<u16>(buffer, CharCode.MINUS);
