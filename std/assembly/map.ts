...
export class Map<K, V> {
  ...
  private find(key: K, hashCode: u32): MapEntry<K, V> | null {
    let entry = load<MapEntry<K, V>>(changetype<usize>(this.buckets) + <usize>(hashCode & this.bucketsMask) * BUCKET_SIZE); // unmanaged!
    while (entry) {
  ...
  delete(key: K): bool {
    ...
    if (halfBucketsMask + 1 >= max<u32>(INITIAL_CAPACITY, this.entriesCount) && this.entriesCount < (this.entriesCapacity * FREE_FACTOR_N) / FREE_FACTOR_D)
